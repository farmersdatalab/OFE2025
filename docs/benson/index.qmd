...
bens_yield <- read_csv(file.path(data_dir, yield_file), show_col_types = FALSE) |>
  rename(yield_mgha = Yield) |>
  mutate(
    yield_buac = yield_mgha * 14.87,
...
yield_summary <- bens_yield |>
  ...
  summarise(
    n          = n(),
    mean_mgha  = round(mean(yield_mgha,  na.rm = TRUE), 3),
    median_mgha= round(median(yield_mgha,na.rm = TRUE), 3),
    sd_mgha    = round(sd(yield_mgha,    na.rm = TRUE), 3),
...
section_cards <- function(mean_cover, mean_ctrl, delta, pct, p_val,
  ...
  <div class="sc-row">
    <div class="sc" style="border-top: 4px solid ', color_cover, '">
      <div class="sc-label">Cover (Treatment)</div>
      <div class="sc-main">
        <span class="sc-val">', fmt(mean_cover), '</span>
        <span class="sc-delta">', sign_fmt(delta), ' vs control (', pct, '%)</span>
      </div>
...
cn_cards <- function(mean_cover, mean_nocover, pct_inc, cohen_d, label_metric, note) {
  ...
  <div class="sc-row">
    <div class="sc" style="border-top: 4px solid #0d9488">
      <div class="sc-label">Cover (Treatment) — ', label_metric, '</div>
      <div class="sc-main">
        <span class="sc-val">', mean_cover, '</span>
        <span class="sc-delta">', sign_fmt(pct_inc), '% vs Control</span>
      </div>
...
<div class="lab-topbar">
  <div class="lab-topbar__inner">
    ...
    <div class="lab-topbar__right">
      <a class="btn btn-download" href="benson_index.pdf" title="Download PDF version">
        ⬇ Download PDF
      </a>
    </div>
  </div>
...
::: {.page-intro}
The map below shows the **corn yield spatial distribution** across the Kolb2 field, displayed in quantiles. The line separates the clover cover crop (treatment) and control sub-fields.
:::
...
<figure style="margin: 20px 0; text-align: center;">
  ...
  />
  <figcaption style="font-size: 0.82rem; color: #6b7280; margin-top: 8px;">
    <strong>Figure 2.</strong> Corn yield map — spatial distribution by quantile.
  </figcaption>
</figure>
...
::: {.page-intro}
The **Nitrogen Balance Index (NBI)** is measured with the Dualex sensor and reflects the ratio of chlorophyll to flavonoids in the leaf. A higher NBI indicates better nitrogen status: the plant has enough N to build chlorophyll without accumulating excess flavonoids as a stress response. Two leaves per plant were measured and averaged. Results are compared between Control (no cover crop) and Treatment (clover cover crop) strips using a two-sample t-test.
:::
...
p <- ggplot(df_avg, aes(x=treatment, y=nbi, fill=treatment, color=treatment)) +
  ...
  scale_color_manual(values = pal,
                     labels = c("control"="Control","treatment"="Cover (Clover)")) +
  scale_x_discrete(labels = c("control"="Control","treatment"="Cover (Clover)")) +
...
::: {.page-intro}
Corn biomass was collected and sent for laboratory C:N analysis. This measures the actual nitrogen concentration in the plant tissue, a direct indicator of how much N the plant was able to take up during the critical early growth window. **Note:** sample sizes are small (n = `r n_nocover_bio` Control, n = `r n_cover_bio` Treatment), so results are descriptive. Effect sizes (Cohen's d) are reported instead of p-values.
:::
...
::: {.page-intro}
Grain samples were collected and analyzed for C:N content. Nitrogen concentration in grain reflects the plant's ability to remobilize N during grain fill, a final-season indicator of whether the cover crop influenced overall nitrogen availability through the growing season.
:::
...
p_n_gr <- ggplot(mean_data_N_gr, aes(x = Treatment, y = mean_N, fill = Treatment)) +
  ...
  scale_y_continuous(expand = expansion(mult = c(0, 0.2))) +
  labs(
    title    = "Nitrogen in Corn Grain (Mean %N ± SE)",
...
p_box_N_gr <- ggplot(bens_grain, aes(x = Treatment, y = TotalN_pct,
  ...
  scale_fill_manual(values  = pal_cn) +
  scale_color_manual(values = pal_cn) +
  labs(title = "Grain %N by Treatment",
...
p_raw_N_gr <- ggplot(bens_grain, aes(x = Treatment, y = TotalN_pct,
  ...
  scale_color_manual(values = pal_cn) +
  scale_fill_manual(values  = pal_cn) +
  labs(title = "Individual Sample Values — Grain %N",
       subtitle = "Horizontal bar = group mean",
       x = "Treatment", y = "Total Nitrogen (%)") +
  theme_minimal(base_size = 13) +
  theme(legend.position = "none", plot.title = element_text(face = "bold"),
...
::: {.page-intro}
The Cornstalk Nitrate Test (CSNT) measures nitrate concentration in the lower cornstalk at the end of the season. It is a post-harvest diagnostic tool: values **above 250 ppm** suggest the crop had sufficient nitrogen; values **below 250 ppm** suggest nitrogen was limiting. For Kolb2 field, **both the control and cover treatment fell below 250 ppm**, indicating that nitrogen was limiting in both strips. Notably, the cover treatment measured *lower* than the control (57 vs 74 ppm), though both are in the same deficient range. Because only **one sample per treatment** was collected, no statistical comparison is possible — this is a field-level observation.
:::
...
<div class="sc-row">
  <div class="sc" style="border-top: 4px solid #e879a0">
    ...
    <div class="sc-sub">⚠ Below 250 ppm threshold — N was limiting</div>
  </div>
  <div class="sc" style="border-top: 4px solid #0d9488">
    <div class="sc-label">Cover (Treatment) — CSNT</div>
    <div class="sc-main">
      <span class="sc-val">', csnt_cover_lab, '</span>
    </div>
    <div class="sc-sub">⚠ Below 250 ppm threshold — N was limiting &nbsp;|&nbsp; n = 1 per treatment</div>
  </div>
</div>
'))
```
...
p_csnt <- ggplot(bens_csnt, aes(x = Treatment, y = CSNT_ppm, fill = Treatment)) +
  ...
  annotate("text", x = 0.55, y = 275, label = "Sufficient N threshold: 250 ppm",
...
<div class="conclusion-grid">
  ...
  <div class="conc-card teal">
    ...
    Cover crop plots showed a <strong>', pct_nbi, '% difference in NBI</strong>
    compared to control (', m_cover_nbi, ' vs ', m_ctrl_nbi, ', ', p_nbi, ').
    Chlorophyll also differed by <strong>', pct_chl, '%</strong> (', p_chl, ').
    These in-season readings capture the nitrogen status.
  </div>
  ...
  <div class="conc-card amber">
    ...
    <strong>', sign_fmt(perc_N_bio), '% difference in tissue nitrogen</strong>
    (', mean_cover_N_bio, '% vs ', mean_nocover_N_bio, '%).
    Cohen\'s d = ', round(d_N_bio$Cohens_d, 2), '. Small sample size
    (n = ', n_nocover_bio, ' vs ', n_cover_bio, ') — descriptive only.
  </div>

  <div class="conc-card">
    <div class="conc-title">🌾 Grain Nitrogen (%N)</div>
    At harvest, grain %N was <strong>', sign_fmt(perc_N_gr), '% different</strong>
    in cover strips (', mean_cover_N_gr, '% vs ', mean_nocover_N_gr, '%,
    ', p_N_gr, ', n = ', n_nocover_gr, ' vs ', n_cover_gr, '). Grain N
    reflects the full-season nitrogen story, integrating both applied
    fertilizer and any cover crop contribution.
  </div>

  <div class="conc-card red">
    <div class="conc-title">🌽 CSNT — N Limiting in Both Strips</div>
    End-of-season CSNT values were below 250 ppm in both treatments
    (Control: ', csnt_ctrl, ' ppm; Cover: ', csnt_cover, ' ppm).
    This indicates the corn crop was nitrogen-limited regardless of
    the cover crop treatment. The 60 lb/ac clover N target was not
    demonstrably met this season — direct clover biomass sampling
    is needed to quantify actual N supply in 2026.
  </div>

  <div class="conc-card teal">
    <div class="conc-title">🌽 Corn Yield — Cover vs No Cover</div>
    Yield monitor data showed cover crop strips averaged
    <strong>', mean_cover_yield, ' bu/ac</strong> vs
    <strong>', mean_nocover_yield, ' bu/ac</strong> in control strips
    — a difference of <strong>+', delta_yield, ' bu/ac (+', pct_yield, '%)</strong>.
    Given the spatial autocorrelation inherent in yield monitor data,
    this is a descriptive field observation rather than a formal
    statistical result, but the direction is consistent with the
    in-season nitrogen indicators.
  </div>
