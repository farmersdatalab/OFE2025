...
summary_df <- tibble(
  ...
  `ANOVA p-value` = c(p_lab_nbi, p_lab_chl, p_lab_cn, p_lab_csnt),
  `Zero (0 lb N)` = c(
...
```{r cn-scatter, echo=FALSE, message=FALSE, warning=FALSE, fig.height=3.5}
p <- ggplot(mac_cn, aes(x = Nrate, y = TotalN_pct, color = NGroup)) +
  geom_point(size = 3, alpha = 0.75) +
  geom_smooth(aes(group = 1), method = "lm", se = TRUE,
              color = "gray40", linetype = "dashed", linewidth = 0.9) +
  scale_color_manual(values = pal4) +
  scale_x_continuous(breaks = c(0, 30, 90, 150)) +
  labs(
    title    = "Biomass %N vs N Rate Applied",
    subtitle = paste0("Linear regression: R² = ", r2_cn, " · ", p_lm_lab),
    x = "N Rate Applied (lb N/ac)", y = "Total Nitrogen in Biomass (%)",
    color = "N Group",
    caption = "Each point = one sub-sample. Dashed line = linear trend ± 95% CI."
  ) +
  theme_minimal(base_size = 13) +
  theme(plot.title = element_text(face = "bold"), legend.position = "bottom",
        panel.grid.minor = element_blank())

ggplotly(p) |> config(displaylogo = FALSE) |> layout(legend = list(orientation = "h", x = 0.5, xanchor = "center", y = 1.05, yanchor = "bottom"))
```
...
```{r csnt-scatter, echo=FALSE, message=FALSE, warning=FALSE, fig.height=3.5}
p <- ggplot(mac_csnt, aes(x = Nrate, y = CSNT_ppm, color = NGroup)) +
  geom_point(size = 3, alpha = 0.75) +
  geom_smooth(aes(group = 1), method = "lm", se = TRUE,
              color = "gray40", linetype = "dashed", linewidth = 0.9) +
  geom_hline(yintercept = 250, linetype = "dotted", color = "red", linewidth = 0.8) +
  scale_color_manual(values = pal4) +
  scale_x_continuous(breaks = c(0, 30, 90, 150)) +
  labs(
    title    = "CSNT vs N Rate Applied",
    subtitle = "Dashed = linear trend · Red dotted = 250 ppm threshold",
    x = "N Rate Applied (lb N/ac)", y = "CSNT (ppm)",
    color = "N Group",
    caption = "Each point = one stalk sample (E or W position)."
  ) +
  theme_minimal(base_size = 13) +
  theme(plot.title = element_text(face = "bold"), legend.position = "bottom",
        panel.grid.minor = element_blank())
