...
nrates <- data.frame(
  ...
  NGroup   = factor(trimws(nrates_raw$Treatment),
                    levels = c("Zero", "Low", "Standard", "High")),
  stringsAsFactors = FALSE
)

# ============================================================
#  YIELD — LECTURA Y PARSE DESDE PlotName (mac_BLK{b}_{t})
#  TrtNum: 1=Zero(0), 2=Low(30), 3=Standard(90), 4=High(150)
# ============================================================
...
cld_yield  <- cld(emm_yield, Letters = letters, adjust = "tukey") |>
  as_tibble() |>
  mutate(
    NGroup = factor(NGroup, levels = c("Zero", "Low", "Standard", "High")),
...
#  Cols: date, field, treatment, group, meas, leaf, block_id, chl, flav, anth, nbi
# ============================================================
...
df_clean <- df_raw |>
  mutate(
    ...
    treatment = factor(treatment, levels = c("Zero", "Low", "Standard", "High")),
    block_id  = factor(str_extract(block_id, "BLK[0-9]+"))
  ) |>
  filter(!is.na(chl), is.na(nbi) | nbi < 150) |>
  arrange(group, meas) |>
  group_by(group) |>
...
df_avg <- df_clean |>
  group_by(date, field, treatment, group, block_id, plant_id) |>
  summarise(
...
# ============================================================
aov_nbi <- aov(nbi ~ treatment + block_id, data = df_avg)
...
aov_chl <- aov(chl ~ treatment + block_id, data = df_avg)
...
This report summarizes the on-farm nitrogen rate trial conducted in the Ntrials field. Four nitrogen rates were tested: 0 (Zero), 30 (Low), 90 (Standard rate), and 150 (High) lb N/ac, in a randomized complete block design with 4 blocks. Statistical comparisons use a randomized complete block design (RCBD) ANOVA with Tukey post-hoc tests. Letters in the figures indicate treatment groups that are significantly different from each other (p < 0.05).
:::
...
<div style="margin: 16px 0 24px; text-align: center;">
  <a href="https://farmersdatalab.github.io/e7a2c4f9-1d3b-4a6e-9c2f-8b5d7e1a0c33/"
     target="_blank"
...
::: {.page-intro}
**Take-home message:** Take-home message: No matter how much nitrogen we applied, from zero all 
the way up to 150 lb/ac, yield, plant color, and end-of-season stalk nitrate were virtually identical across all strips. Your soil appears to be supplying nitrogen on its own, well beyond what the crop needed this season. Based on this data, you may be applying more nitrogen than necessary. Continuing with cover crops remains a smart long-term investment to keep building soil health, even if the short-term nitrogen response isn't showing up yet.
:::
...
summary_df <- tibble(
  ...
  `Standard (90 lb N)` = c(
    fmt1(round(filter(yield_summary, NGroup == "Standard")$mean_yield, 1)),
    fmt1(round(filter(cld_nbi,     treatment == "Standard")$emmean, 1)),
    fmt1(round(filter(cld_chl,     treatment == "Standard")$emmean, 1)),
    fmt1(round(filter(cn_summary,  NGroup    == "Standard")$mean_N, 1)),
    fmt1(round(filter(csnt_summary,NGroup    == "Standard")$mean_csnt, 0))
  ),
  `High (150 lb N)` = c(
...
Means shown. Letters from Tukey HSD test shown in individual section figures. ANOVA: RCBD, 4 blocks × 4 treatments.
:::
...
::: {.page-intro}
We measured corn yield in each strip and matched it to the nitrogen (N) rate used. Then we **compared the average yields** across the four N rates (0, 30, 90, and 150 lb N) to see if any rate clearly outperformed the others. We also **grouped the results** to show which N rates are **truly different from each other**, versus differences that are **just normal field variation** (block design-RCBD). Strips sharing the same letter are statistically the same (Tukey HSD). Finally, we tested whether yield increased steadily with more N applied (linear regression).
:::
...
p <- ggplot(mac_yield, aes(x = NGroup, y = Yield_buAc,
  ...
  scale_fill_manual(values  = pal4) +
  labs(title = "Individual Strip Yields",
...
::: {.page-intro}
The **Nitrogen Balance Index (NBI)** is measured with the Dualex sensor and gives an idea of how much nitrogen the plant has. It compares leaf greenness to stress signals in the leaf, so **higher numbers mean the plant has enough nitrogen**. We took two leaf readings per plant and averaged them. Then we compared the average NBI across the four N rates (0, 30, 90, and 150 lb N) **to see if adding more nitrogen improved plant status**. We also grouped the results to show which rates are truly different, versus differences that are just normal field variation (block design-RCBD). Strips sharing the same letter are statistically the same(Tukey HSD).
:::
...
anova_cards(p_lab_nbi,
            ...
            units = "",
            note = "RCBD ANOVA across Zero, Low, Standard, High")
```
...
::: {.page-intro}
The **Chlorophyll Index (Chl)** estimates chlorophyll content in the leaf, a reliable proxy for nitrogen sufficiency. Greener leaves generally mean more nitrogen in the plant. As with NBI, two leaves per plant were averaged. We also grouped the results to show which rates are truly different, versus differences that are just normal field variation (block design-RCBD). Strips sharing the same letter are statistically the same(Tukey HSD).
:::
...
::: {.page-intro}
We collected corn plant samples from each to measure how much nitrogen actually ended up inside the plant tissue. This is one of the most direct measurements we have, it tells you what the crop actually absorbed, not just what was available in the soil. Each of the 16 plots had two sub-samples averaged together, and we used the same block design as the other analyses to keep the comparison fair. We also ran a linear regression to see if tissue nitrogen climbed steadily with more N applied.
:::
...
::: {.page-intro}
The Cornstalk Nitrate Test (CSNT) measures nitrate concentration in the lower cornstalk at the end of the season. It is a post-harvest diagnostic tool: values **below 250 ppm indicate insufficient nitrogen**, while the **optimal range is between 750 and 2000 ppm**. Each plot had two samples (E and W positions), giving **`r nrow(mac_csnt)` observations** across the 16 plots. The red dashed line marks the 250 ppm deficiency threshold and the green line the 750 ppm optimal threshold.
:::
...
anova_cards(p_lab_csnt,
            ...
            units = "ppm",
            note = "RCBD ANOVA — E and W samples averaged per plot")
```
...
::: {.page-intro}
Soil health was assessed using the **Cornell Soil Health Test** (sampled 04/30/2025, Ntrials field). The overall quality score was **57/100 (Medium)**. The biological indicators are the main area of concern: soil respiration, organic matter, active carbon, and soil protein index all came back Low. These are the indicators that reflect how active and alive your soil biology is, and right now, that biological engine is running at a lower capacity than ideal. The interesting thing is that despite these low biological scores, your field clearly had plenty of nitrogen available this season. That nitrogen is likely coming from a large existing reserve in the soil, not from active biological cycling. Cover cropping is a direct investment in improving exactly these biological scores over time, feeding the microbes, adding carbon, and building a soil that can supply nitrogen more consistently from its own internal processes.
:::
...
::: {.page-intro}
**Cumulative precipitation** at your field compared to the all-site average, alongside the weekly drought index from June through November 2025. Your field tracked closely with the all-site average through the summer months, 
with consistent rainfall from June through August and a cumulative total near the network average heading into September. A strong rain event in late June provided a good early push for the crop. The second half of the season brought a notable dry stretch, drought conditions escalated from Low in late September to Mild for most of October, lasting roughly four to five weeks before easing 
in November. This late-season dry period coincided with the end of grain fill and is worth keeping in mind when interpreting end-of-season stalk nitrate results, as dry conditions can concentrate nitrate in the stalk even when overall nitrogen supply is adequate.
:::
```{=html}
<figure style="margin: 0 0 24px; text-align: center;">
  <img
    src="images/macauley_weather.png"
...
<div class="conc-card teal">
  ...
  Yield was similar across all four N rates, ranging from <strong>153.8 to 
167.7 bu/ac</strong>. No clear pattern emerged as nitrogen increased — the 
strips that got zero fertilizer performed just as well as the ones that got 
150 lb N/ac. The soil appears to have supplied all the nitrogen the crop 
needed on its own this season.
</div>

<div class="conc-card teal">
  <div class="conc-title">🌿 NBI</div>
  NBI values were similar across all treatments (86–92), all within a healthy range.
  Adding more nitrogen did not improve leaf nitrogen status, indicating plants were
  well-supplied regardless of N rate.
</div>

<div class="conc-card">
  <div class="conc-title">🍃 Chlorophyll</div>
  Chlorophyll was nearly identical across all N rates (46–48), confirming that leaf
  greenness was not affected by how much nitrogen was applied. Plants showed no signs
  of nitrogen stress even at zero applied N.
</div>

<div class="conc-card amber">
  <div class="conc-title">🧪 Biomass %N</div>
  Tissue nitrogen was consistent across treatments (4.1–4.2%), with no meaningful
  difference between the zero and high N rate strips. N rate had no detectable
  effect on how much nitrogen the plant accumulated at this growth stage.
</div>

<div class="conc-card pink">
  <div class="conc-title">🌽 CSNT</div>
  End-of-season stalk nitrate was very high in all treatments (4,588–4,818 ppm),
  well above the 750 ppm optimal threshold. The crop had abundant nitrogen at
  season end regardless of how much was applied.
</div>

<div class="conc-card">
  <div class="conc-title">🌱 Soil Health</div>
  The overall soil health score was <strong>57/100 (Medium)</strong>, with low
  biological activity across the board. Despite this, the field appears to have
  a large reserve of available nitrogen. Continued cover cropping could improve
  organic matter and microbial activity over time, even if a single-season
  nitrogen response is not yet visible.
</div>
'))
```
