...
anova_cards(p_lab_nbi,
            ...
            units = "",
            note = "One-way ANOVA across Zero, Low, Standard, High")
```
...
::: {.page-intro}
The **Chlorophyll Index (Chl)** estimates chlorophyll content in the leaf, a reliable proxy for nitrogen sufficiency. As with NBI, two leaves per plant were averaged. We also grouped the results to show which rates are truly different, versus differences that are just normal field variation (block design-RCBD). Strips sharing the same letter are statistically the same(Tukey HSD).
:::
...
::: {.page-intro}
Corn biomass was collected and analyzed for total nitrogen content. Each plot had two sub-samples (A and B), giving **`r nrow(mac_cn)` observations** across the 16 plots (4 blocks × 4 N rates). We used a randomized complete block design (RCBD) to test whether tissue nitrogen differed by N rate, accounting for field spatial variation. A linear regression also tests for a steady dose-response pattern as N rate increases.
:::
...
<div class="conclusion-grid">
  ...
  <div class="conc-card teal">
    ...
    with the Standard rate at <strong>', round(filter(yield_summary, NGroup=="Standard")$mean_yield,1), ' bu/ac</strong>.
  ...
  <div class="conc-card teal">
    ...
    with the Standard rate at <strong>', nbi_farmer, '</strong>.
  ...
  <div class="conc-card">
    ...
    Chlorophyll values were ', chl_zero, ' (Zero), ', chl_farmer, ' (Standard), and ', chl_high, ' (High).
...
The linear regression between %N and N rate showed R² = ', r2_cn, ' (', p_lm_lab, '),
  ...
  <div class="conc-card pink">
    ...
    End-of-season stalk nitrate averaged ', csnt_zero, ' ppm (Zero), ', csnt_farmer, ' ppm (Standard),
