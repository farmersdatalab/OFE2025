...
::: {.page-intro}
Cover crop biomass was sampled across field 302-3 in spring. The %N and C:N ratio of the cover crop residue indicate how quickly nutrients will be released to the following corn crop. A C:N ratio below ~25 suggests relatively fast N release during decomposition.
:::

```{r cc-bar, echo=FALSE, message=FALSE, warning=FALSE, fig.height=4}
bra_cc_plot <- bra_cc |>
  mutate(
    SampleID_num = as.integer(SampleID),
    SampleID_ord = factor(SampleID_num, levels = sort(unique(SampleID_num))),
    Zone = case_when(
  trimws(Treatment) == "Control" ~ "Control",
  trimws(Treatment) == "R3"  	~ "25 lb/ac",
  trimws(Treatment) == "R5"  	~ "50 lb/ac",
  trimws(Treatment) == "R6"  	~ "Variable",
  TRUE                       	~ NA_character_
))
...
p <- ggplot(bra_cc_plot, aes(x = SampleID_ord, y = TotalN_pct, fill = Zone)) +
  geom_col(color = "black", alpha = 0.9, width = 0.7) +
  scale_fill_manual(values = zone_colors) +
  scale_y_continuous(expand = expansion(mult = c(0, 0.15))) +
  labs(
    title   = "Cover Crop Biomass %N by Sample",
    subtitle = "All samples are cover crop — entire field had cover crop",
    x = "Sample ID (numeric order)", y = "Total Nitrogen (%)",
    fill = NULL,
    caption = "Color indicates where in the field the sample was taken (zone, not treatment type)."
  ) +
  theme_minimal(base_size = 13) +
  theme(plot.title = element_text(face = "bold"),
        panel.grid.minor = element_blank())
