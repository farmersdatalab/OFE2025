...
<figure style="margin: 20px 0; text-align: center;">
  ...
  <figcaption style="font-size: 0.82rem; color: #6b7280; margin-top: 8px;">
    <strong>Figure 1.</strong> Porter Field G7 — treatment layout.
...
::: {.page-intro}
**Take-home message:** Across the four nitrogen indicators — NBI, Chlorophyll Index, Biomass %N, and CSNT — treatment plots showed mixed responses compared to the control. The treatment increased chlorophyll relative to the control, indicating improved early-season plant vigor, but this did not translate into consistent improvements in overall nitrogen status or end-of-season sufficiency. With both treatments showing adequate nitrogen levels and high soil quality, the marginal benefit of the treatment appears limited under these conditions.
:::
...
::: {.page-intro}
The map below shows the **corn yield spatial distribution** across Field G7, displayed in quantiles. The blue line marks the field boundary and the dashed line separates the treatment strips.
:::
...
p <- ggplot(df_yield, aes(x = yield, fill = treatment, color = treatment)) +
  ...
  scale_fill_manual(values  = pal, labels = c("control" = "Control (Mow)", "treatment" = "Treatment (No-mowing)")) +
  scale_color_manual(values = pal, labels = c("control" = "Control (Mow)", "treatment" = "Treatment (No-mowing)")) +
  labs(
    title    = "Corn Yield Distribution by Treatment",
    subtitle = paste0("Treatment − Control = ", delta_yield, " t/ha (", pct_yield, "%) · ", p_yield,
...
p <- ggplot(df_yield, aes(x = treatment, y = yield, fill = treatment, color = treatment)) +
  ...
  scale_fill_manual(values  = pal, labels = c("control" = "Control (Mow)", "treatment" = "Treatment (No-mowing)")) +
  scale_color_manual(values = pal, labels = c("control" = "Control (Mow)", "treatment" = "Treatment (No-mowing)")) +
  scale_x_discrete(labels = c("control" = "Control (Mow)", "treatment" = "Treatment (No-mowing)")) +
  labs(title = "Corn Yield by Treatment", subtitle = p_yield,
...
p <- ggplot(yield_stats, aes(x = treatment, y = mean, fill = treatment)) +
  ...
  labs(
    title    = "Mean Corn Yield ± SE by Treatment",
    subtitle = paste0("Difference: ", ifelse(delta_yield >= 0, "+", ""), delta_yield,
                      " t/ha (", pct_yield, "%) · ", p_yield),
    x = "Treatment", y = "Yield (t/ha)"
  ) +
  theme_minimal(base_size = 13) +
...
::: {.page-intro}
Soil health was assessed using the **Cornell Soil Health Test**. The overall quality score was **76/100 (High)**. Most indicators were in the High to Very High range. Key areas to monitor are **Surface Hardness** (score 53-Medium), **Subsurface Hardness** (score 51-Medium), and **Active Carbon** (score 52-Medium), all indicating some compaction and biological activity that could be improved.
:::
...
<div class="conclusion-grid">
  ...
  <div class="conc-card teal">
    ...
    Treatment plots showed a <strong>', abs(pct_nbi), '% ', ifelse(delta_nbi >= 0, "higher", "lower"), ' NBI</strong> compared to Control
    (', m_cover_nbi, ' vs ', m_ctrl_nbi, '). This difference was
    (', p_nbi, '), indicating ', ifelse(tt_nbi$p.value < 0.05,
      "a statistically significant difference in in-season nitrogen status between treatments.",
      "no statistically significant difference in in-season nitrogen status between treatments."), '
  </div>

  <div class="conc-card">
    <div class="conc-title">🍃 Chlorophyll Index (Chl)</div>
    Chlorophyll values were ', ifelse(delta_chl >= 0, "higher", "lower"), ' in Treatment plots
    (<strong>', ifelse(delta_chl >= 0, "+", ""), pct_chl, '%</strong>; ', p_chl, ').
    ', ifelse(tt_chl$p.value < 0.05,
      "This statistically significant difference is consistent with improved nitrogen uptake in Treatment plots.",
  ...
  <div class="conc-card amber">
    ...
    <strong>', ifelse(perc_N >= 0, "+", ""), perc_N, '% ', ifelse(perc_N >= 0, "more", "less"), ' nitrogen</strong> in their tissue
    (', mean_cover_N, '% vs ', mean_nocover_N, '%). The effect size (Cohen\'s d =
    ', round(d_N_cn$Cohens_d, 2), ') suggests a ', ifelse(abs(d_N_cn$Cohens_d) > 0.5, "meaningful", "small"), ' biological difference,
  ...
  <div class="conc-card pink">
    ...
    The Cornstalk Nitrate Test showed Control at <strong>', csnt_ctrl, ' ppm</strong>
    and Treatment at <strong>', csnt_cover, ' ppm</strong>
    (threshold: 250 ppm). ', ifelse(csnt_ctrl >= 250 & csnt_cover >= 250,
      "Both treatments had sufficient nitrogen at season end.",
      ifelse(csnt_ctrl < 250 & csnt_cover >= 250,
        "The Treatment strip met the sufficiency threshold while Control did not, suggesting more available N with earlier sod incorporation.",
        ifelse(csnt_ctrl >= 250 & csnt_cover < 250,
          "The Control strip met the sufficiency threshold while Treatment did not.",
          "Neither treatment met the 250 ppm sufficiency threshold."))), '
  </div>

  <div class="conc-card teal">
    <div class="conc-title">🌾 Corn Yield</div>
    Treatment plots yielded <strong>', m_cover_yield, ' t/ha</strong> vs
    <strong>', m_ctrl_yield, ' t/ha</strong> in Control
    (<strong>', ifelse(delta_yield >= 0, "+", ""), pct_yield, '%</strong> difference, ', p_yield, ').
    ', ifelse(tt_yield$p.value < 0.05,
      paste0("The difference is statistically significant. However, with n = ", n_ctrl_yield + n_cover_yield,
             " pixels, even small differences become significant — the ", abs(pct_yield), "% absolute difference is the more meaningful indicator."),
      "The difference is not statistically significant, suggesting yield was similar across treatments under these conditions."), '
  </div>
